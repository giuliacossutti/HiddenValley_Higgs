...
Plot some variables regarding leptons used to tag Z
...
root -l /gfsvol01/atlas/giuliac/HiddenValley_Higgs/tasks/20250520_Task20/Reclustering_Presel.C'("/gfsvol01/atlas/giuliac/condor/20250520_Task20/50K_signal_EJ.root","signal_EJ",kTRUE)'
root -l /gfsvol01/atlas/giuliac/HiddenValley_Higgs/tasks/20250520_Task20/Reclustering_Presel.C'("/gfsvol01/atlas/giuliac/condor/20250520_Task20/50K_bkg_Zjets_q.root","bkg_Zjets_q",kTRUE)'
root -l /gfsvol01/atlas/giuliac/HiddenValley_Higgs/tasks/20250520_Task20/Reclustering_Presel.C'("/gfsvol01/atlas/giuliac/condor/20250520_Task20/50K_bkg_Zjets_g.root","bkg_Zjets_g",kTRUE)'
root -l /gfsvol01/atlas/giuliac/HiddenValley_Higgs/tasks/20250520_Task20/Reclustering_Presel.C'("/gfsvol01/atlas/giuliac/condor/20250520_Task20/50K_bkg_Zjets_gamma.root","bkg_Zjets_gamma",kTRUE)'
root -l /gfsvol01/atlas/giuliac/HiddenValley_Higgs/tasks/20250520_Task20/Reclustering_Presel.C'("/gfsvol01/atlas/giuliac/condor/20250520_Task20/50K_bkg_ZZ.root","bkg_ZZ",kTRUE)'
root -l /gfsvol01/atlas/giuliac/HiddenValley_Higgs/tasks/20250520_Task20/Reclustering_Presel.C'("/gfsvol01/atlas/giuliac/condor/20250520_Task20/50K_bkg_HZ_SM.root","bkg_HZ_SM",kTRUE)'
*/
...
void DrawHist(std::vector<TH1F*> hist, Bool_t Norm, TString XTitle, TString YTitle, std::array<Double_t,4> leglimits, TString legtitle, std::vector<TString> legentry, TString filename, TString sample, TString dir){
...
void Reclustering_Presel(const char *inputFile, const char * outFile,Bool_t Norm=kTRUE)
{
  ...
  // Output name:
  TString outfolder;
  outfolder.Form("/gfsvol01/atlas/giuliac/plots_and_outputs/20250520_Task20/%s_Presel_bsmallmin%d_smallmin%d_",outFile, n_btagged_min, n_small_min);
  ...
  TH1F *Lep2PT = new TH1F("pt_of_first_two_leptons", "", 300, 0.0, 300.);
  TH1F *Lep2PT_mass = new TH1F("pt_of_first_two_leptons_with_invmass_near_mZ", "", 300, 0.0, 300.);
  TH1F *Lep2mass = new TH1F("Invariant_mass_of_first_two_leptons", "", 200, 0.0, 200.);

  TH1F *LepDeltaR = new TH1F("DeltaR_between_first_two_leptons", "", 80, 0.0, 8.);
  TH1F *LepGenDeltaRE = new TH1F("DeltaR_between_Z_electron_daughters", "", 80, 0.0, 8.);
  TH1F *LepGenDeltaRM = new TH1F("DeltaR_between_Z_muon_daughters", "", 80, 0.0, 8.);
  TH1F *LepGenDeltaRT = new TH1F("DeltaR_between_Z_e_mu_daughters_from_tau", "", 80, 0.0, 8.);
  TH1F *LepEta = new TH1F("Eta_of_first_two_leptons", "", 360, -12., 24.);
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry)
  {
    ...
    // Loop over all particles to find Z and its daughters
    for(Int_t partentry = 0; partentry < branchParticle->GetEntries(); ++partentry)
     {
      ...
      // Select the Z decaying into leptons
      if( (part->PID != 23) || (part->D1 == part->D2) ) continue;

      // Z's daughters  
      GenParticle *zD1 = (GenParticle*) branchParticle->At(part->D1);
      GenParticle *zD2 = (GenParticle*) branchParticle->At(part->D2);

      if( (abs(zD1->PID) != 11) & (abs(zD1->PID) != 13) & (abs(zD1->PID) != 15 )) continue;

      // Fill histograms Z->e+e- and Z->mu+mu-
      if( (abs(zD1->PID) == 11) ){ LepGenDeltaRE->Fill( DeltaR(zD1, zD2) ); LepEta->Fill(zD1->Eta); LepEta->Fill(zD2->Eta); break;}
      if( (abs(zD1->PID) == 13) ){ LepGenDeltaRM->Fill( DeltaR(zD1, zD2) ); LepEta->Fill(zD1->Eta); LepEta->Fill(zD2->Eta); break;}

      // Find 2 opposite charge, same flavor leptons e or mu from tau decays
      if( (abs(zD1->PID) == 15) ){

        // Loop on possible decay products of taus to find 1 of 2 opposite charge, same flavor leptons
        for(Int_t i = partentry; i < branchParticle->GetEntries(); ++i){

          GenParticle *candidate1 = (GenParticle*) branchParticle->At(i);
          if( (abs(candidate1->PID) != 11) & (abs(candidate1->PID) != 13)) continue;

          GenParticle *lM1 = (GenParticle*) branchParticle->At(candidate1->M1);
          if( abs(lM1->PID) == 15){

            // Loop on possible decay products of taus to find 2 of 2 opposite charge, same flavor leptons
            for(Int_t j = partentry; j < branchParticle->GetEntries(); ++j){

               GenParticle *candidate2 = (GenParticle*) branchParticle->At(j);
               if( candidate2->PID == -1.*candidate1->PID ){

                 GenParticle *lM2 = (GenParticle*) branchParticle->At(candidate2->M1);
                 if( lM1->PID == -1.*(lM2->PID)){
                   LepGenDeltaRT->Fill( DeltaR(candidate1, candidate2) );
                   LepEta->Fill(candidate1->Eta);
                   LepEta->Fill(candidate2->Eta);
                   break;
                 }
               }
            }// end loop candidate 2
          }
        }// end loop candidate 1
      }
     ...
     if(isZ2l == kFALSE){
       ...
       if(n_mu == 2){
         ...
         if(mu1->Charge != mu2->Charge){
           ...
           Lep2mass->Fill( (*mu1P4 + *mu2P4).M() );

           // Invariant mass near Z mass
           if( abs( (*mu1P4 + *mu2P4).M() - 91.188 ) < tolerance ){

             Lep2PT_mass->Fill( (*mu1P4 + *mu2P4).Pt() );
...
//-----------------------------------------------------------------
  ...
  if(Norm==kFALSE){
    outtextname = outfolder + "NofEv.txt"; 
  }else if(Norm==kTRUE){
    outtextname = outfolder + "NofEv_Norm.txt"; 
  }

  ofstream outtextfile;
  outtextfile.open(outtextname);
  outtextfile << "This file contains Number of Events passing preselection for sample " << outFile << endl;
  outtextfile << "" << endl;
  outtextfile << "Preselection:" << endl;
  outtextfile << "- 2 charged leptons with same flavour, opposite charge, | Invmass - mZ | < " << tolerance << " GeV, ZpT > 20 GeV" << endl;
  outtextfile << "- At least " << n_small_min << " small jets" << endl;
  outtextfile << "- At least " << n_btagged_min << " b-tagged small jets" << endl;
  outtextfile << "" << endl;
  outtextfile << "Number of Events passing preselection: " << NofEv << endl;
  outtextfile.close();
...
//-----------------------------------------------------------------
  ...
  }else if ( (((strcmp(outFile,"signal_EJ")==0) || (strcmp(outFile,"bkg_ZZ")==0) || (strcmp(outFile,"bkg_HZ_SM")==0)) & (n_btagged_min >= 2)) || (strcmp(outFile,"bkg_Zjets_q")==0) || (strcmp(outFile,"bkg_Zjets_g")==0) || (strcmp(outFile,"bkg_Zjets_gamma")==0)){
...
//----------------------------------------------
  ...
  // Legend
  TLegend* legendDR = new TLegend(0.56391,0.560764,0.877193,0.875);
...
//-------------------------------------------------------
  ...
  // Draw Lepton PT histograms
  CustomizeHist(Lep2PT, kFullSquare, kRed, Norm, 0.5);
  CustomizeHist(Lep2PT_mass, kFullSquare, kBlue, Norm, 0.5);
  std::vector<TH1F*> vecPT = {Lep2PT,Lep2PT_mass};
  std::array<Double_t,4> leglimitsPT = {0.260652,0.189236,0.571429,0.501736};
  if((strcmp(outFile,"bkg_Zjets_q")==0) || (strcmp(outFile,"bkg_Zjets_g")==0) || (strcmp(outFile,"bkg_Zjets_gamma")==0)) leglimitsPT = {0.464912,0.623264,0.753133,0.848958};
  std::vector<TString> legentryPT = {"First 2 Leptons",TString::Format("First 2 Leptons with |IM - mZ| < %.0f GeV", tolerance)};
  DrawHist(vecPT,Norm,"p_{T} [GeV/c]","#frac{1}{N} #frac{dN}{dp_{T}} [c/GeV]",leglimitsPT,"pT of",legentryPT,"Lep2PT", outFile, outfolder);

//-------------------------------------------------------

  // Draw Lepton Invariant Mass histogram
  CustomizeHist(Lep2mass, kFullSquare, kBlue, Norm, 0.8);
  std::vector<TH1F*> vecInvmass = {Lep2mass};
  std::array<Double_t,4> leglimitsInvmass = {0.556391,0.652778,0.867168,0.878472};
  std::vector<TString> legentryInvmass = {"First 2 Leptons"};
  DrawHist(vecInvmass,Norm,"Invariant Mass [GeV/c^2]","#frac{1}{N} #frac{dN}{dM_{inv}} [c^2/GeV]",leglimitsInvmass,"Invariant Mass of",legentryInvmass,"Lep2mass",outFile, outfolder);

//-------------------------------------------------------

  // Draw Lepton DeltaR histogram
  CustomizeHist(LepDeltaR, kFullSquare, kRed, Norm, 0.8);
  CustomizeHist(LepGenDeltaRE, kFullSquare, kOrange, Norm, 0.8);
  CustomizeHist(LepGenDeltaRM, kFullSquare, kBlue, Norm, 0.8);
  CustomizeHist(LepGenDeltaRT, kFullSquare, kGreen+2, Norm, 0.8);
  std::vector<TH1F*> vecDeltaR = {LepDeltaR,LepGenDeltaRE,LepGenDeltaRM,LepGenDeltaRT};
  std::array<Double_t,4> leglimitsDeltaR = {0.219298,0.164931,0.530075,0.388889};
  if((strcmp(outFile,"bkg_Zjets_q")==0) || (strcmp(outFile,"bkg_Zjets_g")==0) || (strcmp(outFile,"bkg_Zjets_gamma")==0)) leglimitsDeltaR = {0.515038,0.614583,0.804511,0.838542};
  std::vector<TString> legentryDeltaR = {"First 2 Leptons (passing pre-sel)","Z's e daughters","Z's #mu daughters","e+e- or #mu+#mu- from Z's #tau decay"};
  DrawHist(vecDeltaR,Norm,"#Delta R","#frac{1}{N} #frac{dN}{d #Delta R}",leglimitsDeltaR,"#Delta R between",legentryDeltaR,"LepDeltaR",outFile,outfolder);

//-------------------------------------------------------

  // Draw Eta histogram
  CustomizeHist(LepEta, kFullSquare, kBlue, Norm, 0.8);
  std::vector<TH1F*> vecEta = {LepEta};
  std::array<Double_t,4> leglimitsEta = {0.556391,0.652778,0.867168,0.878472};
  std::vector<TString> legentryEta = {"Z's OS-SF l daughters"};
  DrawHist(vecEta,Norm,"#eta","#frac{1}{N} #frac{dN}{d #eta}",leglimitsEta,"#eta of",legentryEta,"LepEta", outFile,outfolder);
...
//-------------------------------------------------------
  ...
  Lep2PT_mass->Write("",TObject::kOverwrite);
  Lep2mass->Write("",TObject::kOverwrite);
  LepDeltaR->Write("",TObject::kOverwrite);
  LepGenDeltaRE->Write("",TObject::kOverwrite);
  LepGenDeltaRM->Write("",TObject::kOverwrite);
  LepGenDeltaRT->Write("",TObject::kOverwrite);
  LepEta->Write("",TObject::kOverwrite);
}
