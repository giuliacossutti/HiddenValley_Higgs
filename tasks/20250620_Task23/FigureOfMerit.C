...
root -l /gfsvol01/atlas/giuliac/HiddenValley_Higgs/tasks/20250620_Task23/FigureOfMerit.C 
...
void FOM2D_line(TFile* infile,const char *histname,Int_t samplenumber,Double_t firstcutm,Double_t lastcutm,Int_t ncutsm,Double_t firstcutq,Double_t lastcutq,Int_t ncutsq,TString outdir){
  ...
  TString fomtitle;
  fomtitle.Form("2D Linear FOM: %s >= m %s + q",axesnames4,axesnames2);
...
void FigureOfMerit()
{
  ...
  TString filename = "/gfsvol01/atlas/giuliac/plots_and_outputs/20250620_Task23/area_Zjets_MergedHistos.root";

  TFile *infile = new TFile(filename);

  // Output directory
  TString outdir = "/gfsvol01/atlas/giuliac/plots_and_outputs/20250620_Task23/Zjets_Cut3_";
  ...
  FOM1D(infile,"A_DeltaR_between_leading_and_subleading_small_jets",Nsamples,0.4,6.0,29,0.,outdir);
  FOM1D(infile,"B_DeltaR_between_leading_and_subleading_small_jets",Nsamples,0.4,6.0,29,0.,outdir);

  FOM1D(infile,"A_pt_of_first_two_leptons",Nsamples,40.0,200.0,21,600.,outdir);
  FOM1D(infile,"B_pt_of_first_two_leptons",Nsamples,40.0,200.0,21,600.,outdir);

  FOM1D(infile,"A_number_of_btagged_small_jets",Nsamples,-0.5,4.5,6,12.,outdir);
  FOM1D(infile,"B_number_of_btagged_small_jets",Nsamples,-0.5,4.5,6,12.,outdir);

  // Calculate 2D Rectangular FOMs
  FOM2D_rectangle(infile,"A_Nsmall_vs_Nbsmall",Nsamples,2.5,7.5,6,0.5,4.5,5, outdir);
  FOM2D_rectangle(infile,"B_Nsmall_vs_Nbsmall",Nsamples,2.5,9.5,8,0.5,4.5,5, outdir);
