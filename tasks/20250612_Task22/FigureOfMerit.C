...
Histograms have been saved in files in 20250611_Task22.

From inside the /gfsvol01/atlas/giuliac/HiggsTutorial/MG5_aMC_v3_5_6/Delphes directory run with
root -l /gfsvol01/atlas/giuliac/HiddenValley_Higgs/tasks/20250612_Task22/FigureOfMerit.C 
...
void FOM2D_line(TFile* infile,const char *histname,Int_t samplenumber,Double_t firstcutm,Double_t lastcutm,Int_t ncutsm,Double_t firstcutq,Double_t lastcutq,Int_t ncutsq,TString outdir){
  ...
  TString fomtitle;
  fomtitle.Form("2D Linear FOM: %s >= m %s + q",axesnames3,axesnames1);
...
void FigureOfMerit()
{
  ...
  TString filename = "/gfsvol01/atlas/giuliac/plots_and_outputs/20250611_Task22/area_Zjets_MergedHistos.root";

  TFile *infile = new TFile(filename);

  // Output directory
  TString outdir = "/gfsvol01/atlas/giuliac/plots_and_outputs/20250612_Task22/Zjets_";
  ...
  FOM1D(infile,"Invariant_mass_of_all_btagged_small_jets",Nsamples,0.0,40.0,11,800.,outdir);
  FOM1D(infile,"Invariant_mass_of_leading_and_subleading_small_jets",Nsamples,20.0,200.0,10,0.,outdir);

  FOM1D(infile,"DeltaR_between_leading_and_subleading_large_jets",Nsamples,1.0,6.0,26,0.,outdir);
  FOM1D(infile,"DeltaR_between_leading_and_subleading_small_jets",Nsamples,0.4,4.0,19,0.,outdir);

  FOM1D(infile,"2point_Energy_Correlation_div_pT_of_leading_large_jet",Nsamples,0.0,20.0,11,200.,outdir);
  FOM1D(infile,"2point_Energy_Correlation_div_pT_of_leading_small_jet",Nsamples,0.0,20.0,11,200.,outdir);
  FOM1D(infile,"2point_Energy_Correlation_div_pT_of_subleading_large_jet",Nsamples,0.0,20.0,11,200.,outdir);
  FOM1D(infile,"2point_Energy_Correlation_div_pT_of_subleading_small_jet",Nsamples,0.0,20.0,11,200.,outdir);

  FOM1D(infile,"pt_of_first_two_leptons",Nsamples,40.0,200.0,21,600.,outdir);
  FOM1D(infile,"DeltaR_between_first_two_leptons",Nsamples,0.2,3.0,29,0.,outdir+"lowedge_");
  FOM1D(infile,"DeltaR_between_first_two_leptons",Nsamples,2.6,6.4,39,10.,outdir+"highedge_");

  FOM1D(infile,"number_of_large_jets",Nsamples,-0.5,4.5,6,24.,outdir);
  FOM1D(infile,"number_of_constituents_of_leading_large_jet",Nsamples,0.5,4.5,5,24.,outdir);
  FOM1D(infile,"number_of_small_jets",Nsamples,1.5,9.5,9,24.,outdir);
  FOM1D(infile,"number_of_btagged_small_jets",Nsamples,-0.5,4.5,6,24.,outdir);

  FOM1D(infile,"Prompt_Track_Fraction_of_leading_btagged_small_jet",Nsamples,0.0,0.50,26,-0.5,outdir);
  FOM1D(infile,"Prompt_Track_Fraction_of_leading_large_jet",Nsamples,0.0,0.70,36,-0.5,outdir);
  FOM1D(infile,"Prompt_Track_Fraction_of_leading_small_jet",Nsamples,0.0,0.50,26,-0.5,outdir);
  FOM1D(infile,"Prompt_Track_Fraction_of_subleading_large_jet",Nsamples,0.0,0.70,36,-0.5,outdir);
  FOM1D(infile,"Prompt_Track_Fraction_of_subleading_small_jet",Nsamples,0.0,0.50,26,-0.5,outdir);

  // Calculate 2D Rectangular FOMs
  //FOM2D_rectangle(infile,"JetPT1_vs_Invmass",3,300.,1200.,19,400.,2500.,22, outdir);

  // Calculate 2D Linear FOMs
  //FOM2D_line(infile,"JetPT1_vs_NConst",3,-0.01,0.,101,3.,10.,15, outdir);
