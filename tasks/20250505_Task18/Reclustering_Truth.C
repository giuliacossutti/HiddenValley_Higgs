...
Macro to plot number of genjets with R=0.4 and reclustered genjets with R=1.0
Macro to plot Invariant Mass of genjets with R=0.4 and reclustered genjets with R=1.0
Macro to plot DeltaR, DeltaEta, DeltaPhi between genjets with R=0.4 and reclustered genjets with R=1.0
...
root -l /gfsvol01/atlas/giuliac/HiddenValley_Higgs/tasks/20250505_Task18/Reclustering_Truth.C'("/gfsvol01/atlas/giuliac/condor/20250505_Task18/50K_Higgs.root","Higgs",kTRUE)'
root -l /gfsvol01/atlas/giuliac/HiddenValley_Higgs/tasks/20250505_Task18/Reclustering_Truth.C'("/gfsvol01/atlas/giuliac/condor/20250505_Task18/50K_ReclpTmin40.root","ReclpTmin40",kTRUE)'
root -l /gfsvol01/atlas/giuliac/HiddenValley_Higgs/tasks/20250505_Task18/Reclustering_Truth.C'("/gfsvol01/atlas/giuliac/condor/20250505_Task18/50K_bbbar.root","bbbar",kTRUE)'
*/
...
void Reclustering_Truth(const char *inputFile, const char * outFile,Bool_t Norm=kFALSE)
{
  // No Event selection or Small and Large Jet definition: 

  // Output name:
  TString outfolder;
  outfolder.Form("/gfsvol01/atlas/giuliac/plots_and_outputs/20250505_Task18/%s_Gen_",outFile);
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry)
  {
    ...
    treeReader->ReadEntry(entry);
     ...
     if(index_small >= 2){
       Jet *smalljet1 = (Jet*) branchGenJet->At(index_small1);
       Jet *smalljet2 = (Jet*) branchGenJet->At(index_small2);
       ...
       if(index_small >= 4){
         Jet *smalljet3 = (Jet*) branchGenJet->At(index_small3);
         Jet *smalljet4 = (Jet*) branchGenJet->At(index_small4);
...
;
     ...
     if(index_large >= 2){
       Jet *largejet1 = (Jet*) branchReclusteredGenJet->At(index_large1);
       Jet *largejet2 = (Jet*) branchReclusteredGenJet->At(index_large2);
...
//-----------------------------------------------------------------
  ...
  if( (strcmp(outFile,"ReclpTmin40")==0) || (strcmp(outFile,"bbbar")==0) ){
  ...
  legend->SetHeader(TString::Format("#splitline{Model D, c#tau_{#pi_{D}} = 50 mm, %s, Truth}{#splitline{PYTHIA8+DELPHES, #sqrt{s}=13.6 TeV}{Number of Jets or Constituents with Reclustering}}",outFile),"C");
...
//-------------------------------------------------------
  ...
  legendIM->SetHeader(TString::Format("#splitline{Model D, c#tau_{#pi_{D}} = 50 mm, %s}{#splitline{PYTHIA8+DELPHES, #sqrt{s}=13.6 TeV}{Invariant Mass of Jets, Truth}}",outFile),"C");
...
//----------------------------------------------
  ...
  // Show resulting histograms
  if( ((strcmp(outFile,"Higgs")==0) || (strcmp(outFile,"ReclpTmin40")==0) || (strcmp(outFile,"bbbar")==0)) & (Norm==kTRUE) ){
  ...
  legendDR->SetHeader(TString::Format("#splitline{Model D, c#tau_{#pi_{D}} = 50 mm, %s}{#splitline{PYTHIA8+DELPHES, #sqrt{s}=13.6 TeV}{#Delta R between Jets, Truth}}",outFile),"C");
...
//----------------------------------------------
  ...
  legendDEta->SetHeader(TString::Format("#splitline{Model D, c#tau_{#pi_{D}} = 50 mm, %s}{#splitline{PYTHIA8+DELPHES, #sqrt{s}=13.6 TeV}{#Delta #eta between Jets, Truth}}",outFile),"C");
...
//----------------------------------------------
  ...
  legendDPhi->SetHeader(TString::Format("#splitline{Model D, c#tau_{#pi_{D}} = 50 mm, %s}{#splitline{PYTHIA8+DELPHES, #sqrt{s}=13.6 TeV}{#Delta #varphi between Jets, Truth}}",outFile),"C");
...
//-------------------------------------------------------
  ...
  legendPT->SetHeader(TString::Format("#splitline{#splitline{Model D, c#tau_{#pi_{D}} = 50 mm, %s, Truth}{PYTHIA8+DELPHES, #sqrt{s}=13.6 TeV}}{pT of Jets}",outFile),"C");
