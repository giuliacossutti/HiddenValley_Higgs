...
void Reclustering_Presel(const char *inputFile, const char * outFile,Bool_t Norm=kTRUE)
{
  ...
  // Minimum number of btagged small jets
  Int_t n_btagged_min = 1;
  ...
  // Output name:
  TString outfolder;
  outfolder.Form("/gfsvol01/atlas/giuliac/plots_and_outputs/20250513_Task19/%s_Presel%d_",outFile, n_btagged_min);
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry)
  {
    ...
    //-------------------- Preselection -------------------------
     ...
     if( (n_smalljets < 2) || (n_smallbjets < n_btagged_min) ) continue;
